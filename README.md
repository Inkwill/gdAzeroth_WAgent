...
本分控（黑盒交付 · 设计自治）
   ▼
projects/<项目>/WAgent/   ← 概念层切片（display + rule）共享工作区
```
...
- **文字规则集整合**（rulebook，display + rule 复合产物，按需求驱动，非项目默认交付）
...
- **切片输出**：概念层切片（display + rule）→ 共享工作区 `projects/<项目>/WAgent/`
  - display：世界观/叙事/文化/美术/UI 美学呈现层
  - rule：玩法规则（`WAgent/rules/`，核心循环 / 玩家行为框架 / 胜负判定）
  - rulebook：文字规则集（`rulebook.md`，按需求驱动整合 display + rule）
- **消费锚点**：`assets/manifest.yaml`（资源清单，供 PLAgent 桥收录；无实素材时标 `placeholder: true`）
- **直达互调**：切片可被其他分控按需**只读**引用（DAgent 引用 display + rule 作数值建模输入；PLAgent 引用 display + rule + rulebook 作原型资源与规则来源），无需总控中转
...
- 文字规则集整合（rulebook，按需求驱动）

覆盖概念层全流程：世界观架构与设定文档 / 角色·场景·阵营·文化设定 / 叙事与剧情包装 / 玩法规则（核心循环 / 行为框架 / 胜负判定）/ 文字规则集整合 / 美术需求规格与参考图生成 / 音频·UI 美学的风格指导。
...
| 版本号 | `package.json#version` | 当前版本 |
| 能力声明 | `capabilities.yaml` | 接口契约 0.4.6 自持声明（已落盘；display-design / rule-design / rulebook-design 三主能力 + display 子能力） |
